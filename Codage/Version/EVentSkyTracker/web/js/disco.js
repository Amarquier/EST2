/*
 * Script JavaScript pour la section en mode user
 * de l'application smartphone
 * Utilisé par disco.jsp uniquement
 */

/* global imagesNoms, cheminCalern, cheminChili, cheminRef, dateCrt */
...
/*
 * 
 * @returns {undefined}// on relance init à chaque visualisation de la mage
$(document).on("pagecontainershow", init);
...
$(function() {
    ...
    $("#btnNewRef").on("click", initNewRef);
    $("#btnDmdNewRef").on("click", dmdNewRef);
    $("#popupNewRef").on("popupafterclose", popupNewRefInit);
    ...
    $("#popupNewRef").on("popupafterclose", popupSendMail);
...
/* création d'une nouvelle image de référence */
function initNewRef() {
    var srcRef = document.getElementById("imgobs").src;
    $("#imgNewRef").attr('src',  srcRef);
}

function dmdNewRef() {
    var nomGalaxie = $("#nomGalaxie").text();
    var chemin = document.getElementById("imgNewRef").src;
    buf = chemin.split("/");
    telescope = buf[buf.length - 3];
    date = buf[buf.length - 2];
    chemin = "/jpeg/images_SN/" + telescope + "/" + date + "/" + nomGalaxie + ".jpg";
    var imgNom = imagesNoms[position-1];
    
    $("#btnDmdNewRef").hide();
     // transmettre les infos de la proposition de nouvelle référence
    $.ajax({
        url  : 'discovery.jsp?action=refAdd_req',
        type : 'POST',
        data : 'nomGalaxie=' + nomGalaxie + '&chemin=' + chemin,
        dataType : 'html',
        success: function(data) {
            // y a t'il "oui" dans la réponse ? 
            // On affiche la nouvelle image de ref
            if (data.indexOf("oui") !== -1) {
                $("#imgref").attr('src', cheminRef + nomGalaxie + ".jpg");
                imagesNoms[position-1] = setCharAt(imgNom,2,'O');
            }
            else {
                $('#popupNewRefMsg').text("Merci, votre proposition sera analysée.");
            }
        },
        error : function(resultat, statut, erreur) {
            $('#popupNewRefMsg').text("La requête n'a pas pu être prise en compte.");
        }
    });
} 

function popupNewRefInit() {
    $("#btnDmdNewRef").show();
    $('#popupNewRefMsg').text("Etes-vous sur de vouloir proposer cette image comme nouvelle image de référence ?");
}
...
function confirmCandidat() {
    ...
    $.ajax({
        ...
        url  : 'discovery.jsp?action=candidate_req',
        type : 'POST',
        data :    'nomGalaxie=' + nom
                + '&chemin=' + chemin
                + '&certitude=' + certitude
...
/* init de la popup de zoom de l'image de gauche */
...
/* init de la popup de zoom de l'image de droite */
function popupZoomRight() {
    var imgNom = imagesNoms[position-1];
    var nom = imgNom.substring(4, imgNom.length);
    images[1] = cheminRef + nom + ".jpg";
    if (imgNom.charAt(2) === 'N')
        images[1] = "images/pasdImage.jpg";
...
function nouvelleImage(pos) {
    ...
    // un user ewst connecté et éviter la 1er position (bug)
    if (($("#divUser").length && (position > 1))) {
        // transmettre les infos de la derniere image vue par cet utilisateur
        $.ajax({
            url  : 'discovery.jsp?action=lastImg_req',
            type : 'POST',
            data :    'pseudo=' + nom
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            for (i = 0; i < tabInfos.length; i++) {
                ...
                var leChemin = (info.charAt(0) === 'F') ? "/jpeg/images_SN/Tarot_Calern/" : "/jpeg/images_SN/Tarot_Chili/";
...
function stopBlink() {
    clearInterval(timer);
    // on nettoie la popup modif 3 Mars 2017
    $("#imgBlink").attr('src',"images/black.jpg");
}

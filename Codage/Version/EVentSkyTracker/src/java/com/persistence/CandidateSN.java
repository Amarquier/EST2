 ...
 * Cette table stocke les demandes de candidates des utilisateurs
 */
...
public class CandidateSN {
     ...
     */
    static public CandidateSN create(Connection con, String userPseudo, String nomImage, String chemin,
         int certitude, int x, int y, double ra, double dec) throws Exception {
        CandidateSN candidate = new CandidateSN(certitude, x, y, ra, dec,
                                                  userPseudo, nomImage, chemin);
        String queryString =
         "insert into candidateSN (`date`,`certitude`,`x`,`y`,`ra`,`dec`,"
            + " `userPseudo`, `nomImage`, `chemin`) "
            + " values ("
     ...
     */
    public static ArrayList<CandidateSN> getByImage(Connection con, String nomImage, String chemin) 
                                                            throws Exception {
        ArrayList<CandidateSN> candidates = new ArrayList<CandidateSN>();
        String queryString = "select * from candidateSN "
        ...
        while (lResult.next()) {
            ...
            CandidateSN candidate = 
                    new CandidateSN(date,certitude,x,y,ra,dec,userPseudo,nomImage,chemin);
     ...
     * @throws java.lang.Exception
     */
    public static CandidateSN find(Connection con, String nomImage, String chemin, int i) throws Exception {
        String queryString = "select * from candidateSN "
                                + " where nomImage='" + nomImage + "'"
                                    + " and chemin='" + chemin + "'"
                                    + " order by `date` asc";
    ...
    public static CandidateSN find(Connection con, int i) throws Exception {
        String queryString = "select * from candidateSN "
                                    + " order by `date`";
        Statement lStat = con.createStatement(
                                            ResultSet.TYPE_SCROLL_INSENSITIVE, 
    ...
    public static ArrayList<String> usersPseudoForCandidate(Connection con, String nomImage, String chemin) 
        ...
        String queryString = "select `userPseudo` from candidateSN "
                                    + " where nomImage='" + nomImage + "'"
                                    + " and chemin='" + chemin + "'"
                                    + " order by `date`";
        Statement lStat = con.createStatement(
    ...
    public static int size(Connection con) throws Exception {
        String queryString = "select count(*) as count from candidateSN";
        Statement lStat = con.createStatement(
                                            ResultSet.TYPE_SCROLL_INSENSITIVE, 
                                            ResultSet.CONCUR_READ_ONLY);
        ResultSet lResult = lStat.executeQuery(queryString);
     ...
     */
    private CandidateSN(Timestamp date, int certitude, int x, int y, double ra, 
     ...
     */
    private CandidateSN(int certitude, int x, int y, double ra, 
               double dec, String userPseudo, String nomImage, String chemin) {

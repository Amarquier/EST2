...
<%   
    ...
    ArrayList<Candidate> tabCand = null;
    String img = "";
    String trame = "--";
    ...
    /*Récupération des candidates selon si l'utilsiateur est connecté ou non*/
    if(user!=null)
        tabCand = Candidate.getByEvenementAndUser(con, tabEvent.get(evenement).getId(), user.getPseudo());
    else
    /*Récupération des candidates de l'évènement choisis*/
    tabCand = Candidate.getByEvenement(con, tabEvent.get(evenement).getId());
    if(tabCand.size()<=0)
        evenement++;
    else
        break;
    ...
    listImg = Image.getByCandidate(con, tabCand.get(cand).getId());

...
/**
 * C'est un répertoire sur le disque (dans un répertoire evenement)
 * Qui correspond à un point géométrique de l'espace (ra, dec)
 * Ce répertoire contient toutes les images de cette zone de l'espace
 * pour un évènement donné
 * 
 *      hhmmssssSddmmsss    S = [p|m]
 * ex : 13564567m0328127
 *      ra  = 13h56m45.67s
 *      dec = -03d28m12.7s
 */
public class Candidate {
    ...
    */
    public static Candidate getById(Connection con, int id) throws Exception {
     ...
     * @throws java.lang.Exception
     */
    public static ArrayList<Candidate> getByEvenement(Connection con, int eventId) 
                                                            throws Exception {
        ArrayList<Candidate> candidates = new ArrayList<Candidate>();
        String queryString = "select * from candidate "
                                + "where "
                                    + "(select count(*) as count from image "
                                        + "where image.canId = candidate.id) >= 3"
     ...
     * @throws java.lang.Exception
     */
    public static ArrayList<Candidate> getByEvenementAndUser(Connection con, int eventId, String pseudo) 
                                                            throws Exception {
        ArrayList<Candidate> candidates = new ArrayList<Candidate>();
        String queryString = "select * from candidate where"
                                + "(select count(*) from qualification"
                                    + "where qualification.canId = candidate.id and userPseudo =" + pseudo + ")"
                                        + "and (select count(*) from image "
                                    + "where image.canId = candidate.id) >= 3"
                                        + "and eventId='" + eventId + "' order by ra";
        Statement lStat = con.createStatement(
                                            ResultSet.TYPE_SCROLL_INSENSITIVE, 
     ...
     * @throws java.lang.Exception
     */
    public static Candidate find(Connection con, int eventId, int i) throws Exception {
        String queryString = "select * from candidate "
     ...
     * @throws java.lang.Exception
     */
    public static Candidate find(Connection con, int i) throws Exception {
        String queryString = "select * from candidate";
        Statement lStat = con.createStatement(
                                            ResultSet.TYPE_SCROLL_INSENSITIVE, 
                                            ResultSet.CONCUR_READ_ONLY);
    ...
    private static Candidate creerParRequete(ResultSet result) throws Exception {
        ...
        return new Candidate(lId, lChemin, lRa, lDec, lEventId);
    }
     ...
     */
    private Candidate(int id, String chemin, double ra, double dec, int eventId) {
        this.id = id;
        this.chemin = chemin;
        this.ra = ra;
        this.dec = dec;
